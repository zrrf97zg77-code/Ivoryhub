--// IVORY HUB — BLOX FRUITS MOBILE PVP (FIXED v15)
--// Key System (Key: Ivory) + 180° Silent Aimbot + Soru + Macro Builder
--// Mobile Only — No glitches, no freezes
...
KeyFrame.Size = UDim2.fromOffset(300, 120)
KeyFrame.Position = UDim2.new(0.5, -150, 0.4, -60)
...
KeyButton.Position = UDim2.new(0.1, 0, 0, 80)
KeyButton.BackgroundColor3 = GOLD
...
--// FLOATING TOGGLE (hidden until key valid)
...
--// MAIN FRAME (hidden until key valid)
...
AddCard(Home, "Script Version", "v15.0 - Key + 180° Aimbot", "◈")
...
AddToggle(AimbotPage, "Target Line", "Red line to target", function(enabled)
    Aimbot.ShowLine = enabled
end, "📏")
...
AddSlider(CombatPage, "Soru Distance", 5, 30, 15, function(val)
...
AddCard(CreditsPage, "Version", "v15.0 - Key + Aimbot", "▣")
            ...
            for _, model in pairs(enemies:GetChildren()) do
                if model:IsA("Model") and model:FindFirstChild("Humanoid") and model:FindFirstChild("HumanoidRootPart") then
                    ...
                    if character and character:FindFirstChild("HumanoidRootPart") then
                        ...
                        local dist = (root.Position - otherRoot.Position).Magnitude
                        if dist < shortestDistance then shortestDistance = dist; target = model end
                    end
                end
            end
        end
